...
if(!canvas)return;
// Set explicit pixel dimensions before getting context
...
const C={amber:'#EF9F27',amberD:'#A8763B',teal:'#1D9E75',tealD:'#0F6E56',blue:'#378ADD',blueD:'#185FA5',purple:'#7F77DD',purpleD:'#534AB7',gray:'#888780',muted:'#6B665E'};
let SRC=[],MODS=[],EXEC=[],srcX,modX,execX;

function layout(){
  const cw = canvas.parentElement.offsetWidth - 40;
...
function drawBox(b,hl){
  ...
  ctx.fillStyle=hex2rgb(b.col,0.13+hl*0.15);ctx.fill();
  ctx.strokeStyle=hex2rgb(b.colD,0.5+hl*0.4);ctx.lineWidth=dpr*(hl?1.8:0.9);ctx.stroke();
  const fs=Math.max(11,Math.floor(13*dpr/2));
  ...
  ctx.font=`${Math.max(9,Math.floor(10*dpr/2))}px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`;
...
function drawPlatformBox(){
  ...
  ctx.strokeStyle=hex2rgb(C.tealD,0.22);ctx.lineWidth=dpr;
  ctx.setLineDash([5*dpr,4*dpr]);ctx.stroke();ctx.setLineDash([]);
  ctx.fillStyle=hex2rgb(C.tealD,0.45);
  ctx.font=`500 ${Math.max(9,Math.floor(10*dpr/2))}px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`;
  ctx.textAlign='center';ctx.textBaseline='top';
  ctx.fillText('Payment automation platform',m.x,y+5*dpr);
}

function drawConnectors(){
  ctx.setLineDash([4*dpr,4*dpr]);ctx.lineWidth=dpr*0.7;ctx.strokeStyle=hex2rgb(C.gray,0.22);
  SRC.forEach(s=>{
    const m=MODS[0],sx=s.x+s.w/2,sy=bcy(s),tx=m.x-m.w/2,ty=bcy(m);
    ctx.beginPath();ctx.moveTo(sx,sy);
    ctx.bezierCurveTo(sx+W*0.07,sy,tx-W*0.07,ty,tx,ty);ctx.stroke();
  });
...
class Particle{
  constructor(si){
    const s=SRC[si];this.si=si;this.phase=0;this.t=0;
    ...
    this.sx=s.x+s.w/2;this.sy=bcy(s);this.tx=0;this.ty=0;
    this.setTarget();
  }
  setTarget(){
    const m0=MODS[0];
  ...
  draw(){
    ...
    const mx=(this.sx+this.tx)/2,my=(this.sy+this.ty)/2-H*0.035;
    ...
    for(let i=3;i>=0;i--){
      ...
      ctx.fillStyle=hex2rgb(this.col,0.1*(4-i));ctx.fill();
    }
    ...
    ctx.fillStyle=hex2rgb(this.col,0.18);ctx.fill();
  }
}
...
function draw(){
  ...
  SRC.forEach(b=>drawBox(b,0));
  MODS.forEach(b=>drawBox(b,b._hl||0));
  EXEC.forEach(b=>drawBox(b,0));
